...
final class DefaultOftConnection implements OftConnection {
     ...
     * {@link OftSecurityMode#TRUSTED} set) and hail exchange against it, and returns the resulting
     ...
     * certificate against); {@link OftSecurityMode#SERVER_AUTHENTICATION} falls back to the JVM's default
    ...
    static DefaultOftConnection establishAsServer(Socket rawSocket, OftHostOptions options) throws IOException {
        ...
        // SERVER_AUTHENTICATION/DUAL_AUTHENTICATION, it's the caller-supplied one, already validated
     ...
     * inbound packets to {@link #receivedSlot}/{@link #disconnectedSlot}), the send loop, the
     * {@code Poll} timer (see README.md &sect;10), and (if configured, and not {@link #insecure})
     * the automatic rekey timer. Safe to call immediately after establishment, with no need to wait
     * for a caller to assign a callback first: both slots are {@link BufferedHandlerSlot}s, so
     * nothing raised before a callback is assigned is ever lost.
     */
    ...
    @Override
    public void setReceivedHandler(Consumer<byte[]> handler) {
        this.receivedSlot.setHandler(handler);
    }

    @Override
    public Consumer<byte[]> getReceivedHandler() {
        return this.receivedSlot.getHandler();
    }

    @Override
    public void setDisconnectedHandler(Consumer<Throwable> handler) {
        this.disconnectedSlot.setHandler(handler);
    }

    @Override
    public Consumer<Throwable> getDisconnectedHandler() {
        return this.disconnectedSlot.getHandler();
    }
    ...
    private void raiseReceived(byte[] data) {
        this.receivedSlot.raise(handler -> handler.accept(data));
    }
    ...
    private void close(Throwable exception) {
        ...
        this.disconnectedSlot.raise(handler -> handler.accept(exception));

        // Discards anything still buffered for lack of a callback: if nobody ever assigned one,
        // whatever arrived is no longer reachable once the connection is gone.
        this.receivedSlot.clear();
        this.disconnectedSlot.clear();
    }
