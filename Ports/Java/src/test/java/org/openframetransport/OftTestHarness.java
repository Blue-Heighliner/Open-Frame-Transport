...
final class OftTestHarness {
    ...
    static Pair establish(int maxPacketDataSize, Duration rekeyInterval) throws Exception {
        return establish(maxPacketDataSize, rekeyInterval, OftSecurityMode.AUTHENTICATION, Duration.ofSeconds(1), Duration.ofSeconds(5));
    }
    ...
    static Pair establish(int maxPacketDataSize, Duration rekeyInterval, OftSecurityMode securityMode, Duration pollInterval, Duration pollTimeout) throws Exception {
        boolean needsServerContext = securityMode == OftSecurityMode.AUTHENTICATION || securityMode == OftSecurityMode.DUAL_AUTHENTICATION;
    ...
    static final class TrackedListener implements AutoCloseable {
        ...
        private TrackedListener(OftListener listener) {
            ...
            listener.addConnectedListener(this::onConnected);
        }
        ...
        void addConnectedListener(Consumer<OftConnection> listener) {
            this.listener.addConnectedListener(listener);
        }

        private void onConnected(OftConnection connection) {
            this.connections.add(connection);
            connection.addDisconnectedListener(exception -> this.connections.remove(connection));
        }
